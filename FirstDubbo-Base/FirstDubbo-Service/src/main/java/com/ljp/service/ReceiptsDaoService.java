...
import com.ljp.serviceInterface.ReceiptsDaoServiceFace;
import com.ljp.serviceInterface.WorkerServiceFace;

public class ReceiptsDaoService implements ReceiptsDaoServiceFace {

	protected WorkerServiceFace workerService;
	...
	public ReceiptsDaoService(WorkerServiceFace workerService, ReceiptsMapper receiptsMapper,
	...
	public boolean updateReceiptsDetailsByReceiptsId(Integer receiptsId, List<ReceiptsDetails> newItems) {
		...
		for (ReceiptsDetails receiptsDetails : newItems) {
			...
			if (receiptsDetails.getReceiptsDetailsId() == null) {
				...
				System.out.println("����������" + countDettails);
				if (countDettails == 0) {
					return false;
				}
			} else {
				...
				for (int i = 0; i < oldItems.size(); i++) {
					...
					if (receiptsDetails2.getReceiptsDetailsId().intValue() == receiptsDetails.getReceiptsDetailsId()
						...
						System.out.println("ɾ��ģ��oldItems��" + receiptsDetails2);
						break;
					}
				}
			}
		...
		for (ReceiptsDetails receiptsDetails2 : oldItems) {
			...
			System.out.println("ɾ��oldItems���ݿ�������" + deleteByPrimaryKey);
		}
		return true;
	}
